...
RESOURCE_BUDGET = 0.35

# BRAM physical organization
BRAM_PHYSICAL_DEPTH = 512
BRAM_PHYSICAL_WIDTH_BITS = 32 

# Energy Estimation (based on provided table: 8b mul 4.2pJ + 32b add 5.8pJ)
E_MAC_PJ = 10.0                     # INT8 multiply + INT32 accumulate
E_DRAM_PJ_BIT = 12.0                # LPDDR4 access
E_REG_PJ_BIT = 0.05                 # Register file access
E_SRAM_BASE_PJ_BIT = 0.8  

# Static power model
P_STATIC_BASE_W = 3.5
P_STATIC_UTIL_FACTOR = 8.0      

# Frequency model (max 300MHz, min 100MHz)
FREQ_BASE_MHZ = 300.0
FREQ_MIN_MHZ = 100.0
FREQ_PENALTY_COEFF = 0.85       
...
class FlexSparseDSE:
    ...
    def check_resource_constraints(self, hw_cfg: HardwareConfig) -> Tuple[bool, Dict[str, float], str]:
        # Updated LUT per PE: increased to 200 for more realistic resource scaling
        lut_per_pe = 120
        dsp_per_pe = 1                     # INT8 multiplier without DSP
        lut_per_bank_ctrl = 1080 
        ...
        utils = {
            ...
            "dsp": total_dsps / (ZCU102_DSP_TOTAL * RESOURCE_BUDGET),
...
def visualize_dse_results(results: List[Dict]):
    """Visualize DSE results with 3D surface and 2D contour"""
    if not results: 
...
def run_dse_flexsparse_target_model(wls: List[WorkloadConfig]):
    ...
    print("-" * 90)
    if best_config:
        print(f"\nOptimal PE array: {best_config.array_rows-12} x {best_config.array_cols}")
