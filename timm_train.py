...
import compression
from compression import dyna_set_sparse_budget, create_compressed_deit_small
...
parser.add_argument('--data-dir', metavar='DIR', default='/data/ilsvrc12_raw_torch',
...
group.add_argument('--experiment', default='HybirdSparse', type=str, metavar='NAME',
...
group = parser.add_argument_group('Sparse training parameters')
group.add_argument('--sparsity-mode', type=str, default='dense', choices=['dense', 'sparse', 'hybird', 'sparse_quant'],
                   help='Sparsity mode')
group.add_argument('--mask-only', action='store_true', default=False)
group.add_argument('--sparse-checkpoint', type=str, default=None)
...
group.add_argument('--sparse-budget', type=str, default=None)
group.add_argument('--reduced-token', type=int, default=24)
group.add_argument('--n-bits', type=int, default=8)
...
def main():
    ...
    # exclude_layer = [model.get_classifier(), model.head, model.head_dist]
    # exclude_layer = [model[ex] for ex in ['head', 'head_dist'] if hasattr(model, ex)]
    
    if args.sparsity_mode == 'sparse':
        import compression
        exclude_layer = [getattr(model, ex) for ex in ['head', 'head_dist'] if hasattr(model, ex)]
        model = compression.utils.replace_linear_with_(
            model, 
            compression.SparseLinearFrozen, 
            exclude=exclude_layer)
    elif args.sparsity_mode == 'hybird':
        import compression, pickle
        exclude_layer = [getattr(model, ex) for ex in ['head', 'head_dist'] if hasattr(model, ex)]
        model = compression.utils.replace_linear_with_(
            model, 
            compression.SparseLinear, 
            exclude=exclude_layer,
            N=2, M=4, hard=False)  # 
    
        ## Apply sparse budget here
        if args.sparse_budget is not None:
            s_b = pickle.load(open(args.sparse_budget, 'rb'))
            dyna_set_sparse_budget(model, s_b, verbose=True)
            # print(f"Successfully load and set sparse budget from {args.sparse_budget}")
            print_rank0(args, f"Successfully load and set sparse budget from {args.sparse_budget}")
        # compression.utils.dyna_set_sparse_budget(model, args.sparse_budget)
    elif args.sparsity_mode == 'sparse_quant':
        import compression
        pt = torch.load(args.sparse_checkpoint, map_location='cpu')
        ptsd = pt['state_dict']
        assert 'state_dict' not in ptsd.keys(), f"state_dict should not be in the checkpoint"
        model = create_compressed_deit_small(
            pretrained=False,
            n_bits=8,
            reduced_token=args.reduced_token,
            pruning_layer_indices=[3, 6, 9],
            pt_state_dict=ptsd
        )
    else:
        raise ValueError(f"Unknown sparsity mode: {args.sparsity_mode}")
    
    if args.rank==0:
        print(model)
    
    if args.sparse_checkpoint and not args.sparsity_mode == 'sparse_quant':
        print_rank0(args, f'Loading sparse checkpoint from {args.sparse_checkpoint}')
        checkpoint = torch.load(args.sparse_checkpoint, map_location='cpu')
        model.load_state_dict(checkpoint, strict=False if args.sparsity_mode == 'hybird' else True)

        # for learnable mask, load the prior
        for name, module in model.named_modules():
            if isinstance(module, compression.SparseLinear):
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[device], broadcast_buffers=not args.no_ddp_bb, find_unused_parameters=False)  # Fasle
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.sparsity_mode == 'hybird':
                ...
                for m in model.modules():
                    if isinstance(m, compression.SparseLinear):
                        m.tau = tau
                        m.scaling = scaling
